...
void ControlNetwork::Execute(SysCommand *command){
  ...
  if(command->type==SysCommand::kCN){
    ...
    }else if (command->Keyword("dump","general")) {
      interface->dumpgeneral();
    }else if (command->Keyword("set","general", &data)) {
      interface->setgeneral(*data);
