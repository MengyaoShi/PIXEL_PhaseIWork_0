...
//   int setssdi_ = aFECInterface->setssid(0);
    ...
    // send inject trigger through data programming
	
//    int tbmcommand = aFECInterface->tbmcmd(1, 1, 0xE, 0x0C, 0x02, 0x02 , 0x01, 0 );
//    int tbmcommand2 =aFECInterface->tbmcmd(1, 2, 0xE, 0x00, 0x00, 0x00 , 0x00, 0 );
//    int resetroc = aFECInterface->tbmcmd(1, 1, 0xE, 0x00, 0x04, 0x02 , 0x04, 0 );
    
    aFECInterface->fecDebug(2);
    cout <<"reading back "<<endl;
//    int readback = aFECInterface->readback(1,1);
    
    int myData;
//    int readbyte = aFECInterface->getByteHubCount(1, 1, 4, &myData);
    
 //   cout <<"myData " << hex << myData << hex << " "<<readbyte << dec <<  endl;
    
    unsigned long data = 0;
    int ret = aFECInterface->getversion(&data); //
