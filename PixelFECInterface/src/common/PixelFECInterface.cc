...
void PixelFECInterface::setgeneral(unsigned data) {
  pRegManager->WriteReg("GenReg", data);
  valword value = pRegManager->ReadReg("GenReg");
  cout << "SETGENERAL readback  Valid? " << value.valid() << " value " << std::hex << value.value() << std::dec << endl;  
}
void PixelFECInterface::dumpgeneral() {
  valword value = pRegManager->ReadReg("GenReg");
  cout << "DUMPGENERAL  Valid? " << value.valid() << " value " << std::hex << value.value() << std::dec << endl;  
}
//-----------------------------------------------------------------------
...
unsigned int PixelFECInterface::flipByte(unsigned int input){
    uint32_t result = 0;
