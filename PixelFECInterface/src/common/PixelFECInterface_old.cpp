...
int PixelFECInterface::getversion(unsigned long *data) {
    valword value;
    value = pRegManager->ReadReg("VERSIONM08");
    *data = value.value();
...
int PixelFECInterface::outputbuffer(const int mfec, const int fecchannel,
    ...
    switch (mfec) {
        case 1:
            if (fecchannel == 1) {
                outputwordhalblock("OutFIFO.BOUT_BUF1M1", data);
            } else if (fecchannel == 2) {
                outputwordhalblock("OutFIFO.BOUT_BUF2M1", data);
            }
            break;
        case 2:
            if (fecchannel == 1) {
...
void PixelFECInterface::outputwordhalblock(const char *halname, unsigned int data) {
   ...
   // data = flipByte(data);	
    cout << std::hex <<  data << std::hex <<endl;
    cout << std::dec <<  data << std::dec <<endl;
    std::vector<uint32_t> vec;
    vec.push_back(data);
    pRegManager->WriteBlockReg(halname, vec);   // Should 4-byte go to the xml file? mode="block" size="4" />
...
int PixelFECInterface::outputblock(const int mfec, const int fecchannel,
                                   unsigned int *data, int ndata) {return 1;}
...
int PixelFECInterface::tbmcmd(int mfec, int fecchannel,
    ...
    for (i=0;i<ndata;i+=4) {
        iword = (long unsigned int*) &txdata[i];
        outputbuffer(mfec, fecchannel,  *iword);
        if (PRINT) cout<<"Final FEC data (ndata:"<<hex<<ndata<<")  ("<<i<<"): "<< *iword<<" "<<iword <<dec<<endl;
    }
#endif
    // Now data is in txdata.  Ready to initiate tx.  Reset and send go.
    writeCSregister(mfec, fecchannel, 0x07);
